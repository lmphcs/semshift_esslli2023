...
tokenizer = AutoTokenizer.from_pretrained(modelname, never_split=targetforms)
model = AutoModelForMaskedLM.from_pretrained(modelname, output_hidden_states=True)
model.to(device)

# tokenizer.add_tokens(targetforms)
...
dataset = ContextsDataset(ids2lemma, sentences, context_window, tokenizer, len_longest_tokenized,
                          nSentences)
...
output = "token_embeddings_corpus2_bert.npz"
np.savez_compressed(output, **usages)
